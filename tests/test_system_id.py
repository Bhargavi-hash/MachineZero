...
def test_hypotheses_shrink_with_observations():
    ...
    model=HypothesisPredictor(spec.word_bits,spec.num_registers); op=spec.opcodes[0].opcode
    before=len(model.hypotheses(op)); model.fit(ctx); after=len(model.hypotheses(op)); assert after <= before
