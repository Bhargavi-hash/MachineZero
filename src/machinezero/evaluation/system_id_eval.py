...
def evaluate(budgets=(0, 1, 2, 5, 10, 20, 30), queries_per_arch=40):
    ...
    for ename, explorer in [("random", RandomExplorer()), ("coverage", CoverageExplorer())]:
        for budget in budgets:
            sums = {"exact": 0.0, "register_accuracy": 0.0, "flag_accuracy": 0.0}
            count = 0
            for seed in splits.test:
                ...
                rng = random.Random(seed * 101 + budget)
                for _ in range(queries_per_arch):
                    s, ins = sample_query(rng, spec)
                    pred = model.predict(s, ins)
                    actual = cpu.step(s, ins)
                    m = compare_states(pred, actual)
                    for k in sums:
                        sums[k] += m[k]
                    count += 1
            row = {
...
def main():
    ...
    p.add_argument("--out", default="results/system_id_eval.json")
    a = p.parse_args()
    r = evaluate()
