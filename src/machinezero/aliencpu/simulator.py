...
class AlienCPU:
    ...
    def _regs(self, s: OpcodeSpec, ins: Instruction) -> tuple[int, int]:
        a = ins.a % self.spec.num_registers
        b = ins.b % self.spec.num_registers
        return (a, b) if s.dst_first else (b, a)

    def step(self, state: CPUState, ins: Instruction) -> CPUState:
        if ins.opcode not in self._map:
            raise ValueError(f"Unknown opcode {ins.opcode}")
        out = state.clone()
        sp = self._map[ins.opcode]
        m = self.spec.mask
        dst, src = self._regs(sp, ins)
        old = out.registers[dst]
        rhs = out.registers[src]
        result = None
        carry = out.carry
        op = sp.operation
        if op == "MOV":
            ...
            carry = (old >> (self.spec.word_bits - 1)) & 1
            result = (old << 1) & m
        elif op == "SHR":
            carry = old & 1
            result = (old >> 1) & m
            ...
            result = ins.b & m
            dst = ins.a % self.spec.num_registers
        elif op == "COMPARE":
            result = (old - rhs) & m
            carry = int(old >= rhs)
        elif op == "JMP" or (op == "JZ" and out.zero):
            out.pc = ins.a & m
        if result is not None and op != "COMPARE":
            out.registers[dst] = result & m
        if sp.updates_zero and result is not None:
            out.zero = int((result & m) == 0)
        if sp.updates_carry:
            out.carry = carry
        if op not in {"JMP", "JZ"} or out.pc == state.pc:
            out.pc = (state.pc + 1) & m
        return out
    ...
    def run(self, program: list[Instruction], state: CPUState | None = None, max_steps: int | None = None) -> CPUState:
        ...
        while 0 <= cur.pc < len(program) and steps < max_steps:
            cur = self.step(cur, program[cur.pc])
            steps += 1
        return cur
