...
OPS = {
 'MOV':2,'ADD':2,'SUB':2,'XOR':2,'AND':2,'OR':2,'NOT':1,'SHL':1,'SHR':1,
 'LOAD_IMMEDIATE':2,'COMPARE':2,'JMP':1,'JZ':1,
}
...
def generate_architecture(seed: int) -> ArchitectureSpec:
    ...
    has_zero = rng.random() < .85
    has_carry = rng.random() < .55
    ...
    for op, code in zip(chosen, opcode_values):
        specs.append(OpcodeSpec(
            opcode=code, operation=op, arity=OPS[op],
            dst_first=(rng.random() < .5 if OPS[op] == 2 and op not in {'LOAD_IMMEDIATE','JMP','JZ'} else True),
