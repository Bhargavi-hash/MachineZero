...
def main():
 ...
 for epoch in range(tc['epochs']):
  total=0.0
  for batch in dl:
   opt.zero_grad(); y=model(batch['tokens'].to(device),batch['padding_mask'].to(device)); loss=semantic_loss(y,batch['target'].to(device)); loss.backward(); torch.nn.utils.clip_grad_norm_(model.parameters(),1.0); opt.step(); total+=loss.item(); step+=1
