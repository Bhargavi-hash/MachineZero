...
def make_splits(
    base_seed: int = 1000, n_train: int = 64, n_validation: int = 16, n_test: int = 16
) -> ArchitectureSplits:
...
def architecture_ids(seeds: tuple[int, ...]) -> set[str]:
    return {generate_architecture(s).architecture_id for s in seeds}
...
def validate_disjoint(s: ArchitectureSplits) -> None:
    ...
    if a & b or a & c or b & c:
        raise ValueError("Architecture-level split overlap")
