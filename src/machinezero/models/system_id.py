...
class HypothesisPredictor:
    ...
    def hypotheses(self, opcode: int) -> list[OpcodeSpec]:
        ...
        if not obs:
            return list(self._candidate_specs(opcode))
        valid = []
        for cand in self._candidate_specs(opcode):
            cpu = self._cpu(cand)
            if all(self._same(cpu.step(before, ins), after) for before, ins, after in obs):
                valid.append(cand)
        return valid
    ...
    def predict(self, before: CPUState, ins: Instruction) -> CPUState:
        ...
        if not hs:
            out = before.clone()
            out.pc = (out.pc + 1) & ((1 << self.word_bits) - 1)
            return out
    ...
    def summarize(self, opcode: int) -> dict:
        hs = self.hypotheses(opcode)
        ops = Counter(h.operation for h in hs)
        total = max(1, sum(ops.values()))
        best, count = ops.most_common(1)[0] if ops else ("UNKNOWN", 0)
