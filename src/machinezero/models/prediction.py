...
def predict_state(
    ...
    tokens = [
        encode_transition(a, b, c, word_bits, num_registers, query_opcode=ins.opcode)
        for a, b, c in context
    ]
    tokens.append(
        encode_transition(
            before, ins, None, word_bits, num_registers, True, query_opcode=ins.opcode
        )
    )
    ...
    model.eval()
    with torch.no_grad():
        logits = model(x, mask)[0].cpu()
    semantics = decode_semantics(logits)
